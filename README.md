...
| [`curating-context`](skills/curating-context/) | curate context, context budget, hone AGENTS.md, trim AGENTS.md, prune context | Keep `AGENTS.md` and its live reference docs small, true, and navigable: token-budget census, mechanical fact verification, keep/demote/tighten/delete classification, relocate-not-delete edits, and a committed JSONL telemetry ledger with a cross-cohort roll-up. Ships two surfaces that catch regrowth between runs — an installable `PostToolUse` write guard, and a review-time delta the `reviewing-code*` variants call automatically |
