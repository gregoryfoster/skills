...
| [`auditing-ci-cost`](skills/auditing-ci-cost/) | audit CI, CI cost, Actions spend, optimize CI | Audit GitHub Actions spend by measuring the cost shape first: a per-job billed-minute census computed from job timestamps, platform-incident days separated from the structural baseline, then a prescription that branches on job COUNT vs job DURATION — the levers for one are actively wrong for the other. Ships a path-filter replay gated on zero false skips, a required non-levers section, and a handoff to `enforcing-architecture` |
| [`shipping-work`](skills/shipping-work/) | ship it, push GH, close GH, wrap up | Commit, push, comment, and close GitHub issues. **Fallback of last resort** for any stack with no dedicated variant |
...
| [`shipping-work-python-fastapi`](skills/shipping-work-python-fastapi/) | ship it, push GH, close GH, wrap up | Python/FastAPI variant of `shipping-work` (Alembic migrations, systemd service restarts) |
| [`shipping-work-python-click`](skills/shipping-work-python-click/) | ship it, push GH, close GH, wrap up | Python/Click CLI variant of `shipping-work` (Click command registration, `--help` output) |
...
| [`init-socraticode`](skills/init-socraticode/) | init socraticode, set up code search, index this project, socraticode setup | Install, configure, and index SocratiCode semantic code search: host preflight (Docker, or a shared Qdrant store; Node/npx), plugin enablement, a project-adapted Code Exploration Policy + prefetch hook, a context-artifacts manifest, and a blocking index verified green (embeddings + graph + artifacts) |
...
| [`using-mayfly-chat`](skills/using-mayfly-chat/) | mayfly, open a channel, join the channel, chat with <repo>, agent chat | Live agent-to-agent exchange over a Mayfly Chat channel (end-to-end encrypted, ephemeral, the URL is the only credential): decide per question whether a channel is warranted, keep the URL out of argv and every durable store, post from files with compare-and-swap cursor discipline, listen in the background, verify every peer claim, and land outcomes on issues before goodbye |
