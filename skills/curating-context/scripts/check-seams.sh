...
# A fence CLOSES on the marker that opened it. Matching either interchangeably
# means a `~~~` inside a ``` block — a gate's own output, a doc showing example
# markdown — flips the state, and the rest of the file is then keyed by the
# fenced rules: headings compared, prose at the lower floor. The same line would
# be keyed differently in two files, which is the one thing a shared walker
# exists to prevent. This was a divergence from the siblings when #272 CR 13
# made it (no false trigger could be produced from the old toggle — the
# destination side applies its own floor and absorbed the case built for it);
# #275 brought prove-no-loss.sh and check-counts.sh to the same close, and the
# constant is pinned identical across the three by test_heredoc_twins.py.
