...
Exit codes:
  ...
  3  one or more seams to review — judge each, fix what is wrong, re-run
USAGE
}
...
RC=0
python3 - "$TMP/base_policy" "$REL" "$TMP/docs" <<'PY' || RC=$?
import re
import sys

base_policy_path, policy_rel, docs_list = sys.argv[1:4]
...
for k, orig in sweepable.items():
    ...
    for path in [policy_rel] + docs:
        for i, line in enumerate(doc_lines(path), 1):
            ...
            if m and norm_title(m.group(2)) == k:
                continue        # the relocated section's own heading
...
if seams:
    print(f"{len(seams)} seam(s) to review — each needs a decision, not "
          "necessarily a fix:\n")
    width = max(len(c) for c, _, _ in seams)
    ...
    print("\nA back-reference hit is wrong only if the content it points at "
          "moved; judge each,")
    print("fix what lies, then re-run — the count below goes on the ledger row "
          "via --seams.")
else:
    print("OK — no cross-reference seams found.")

print(f"\nseams: {len(seams)}")
sys.exit(3 if seams else 0)
PY
