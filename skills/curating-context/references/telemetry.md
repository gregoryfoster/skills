...
| `docs_unindexed` | live docs reached only through another doc — linked, so not orphans, but named nowhere in the policy file, which is what an agent routes by. `null` on a row predating the field. Reported and **not** gated: an unindexed doc is a routing loss rather than a content loss, and an annex — or a repo whose complete index cannot fit its budget — carries some on purpose ([#274](https://github.com/gregoryfoster/skills/issues/274), [the measurements](cohort-patterns.md#reachable-is-not-routable)). Nothing acknowledges those, so such a repo never reads `0`: watch the **delta**, as with `seams_acked` — a rise is a doc that lost its line or never got one, to be judged |
...
| `claims_dropped`, `claims_warranted` | [`prove-no-loss.sh --claims`](../scripts/prove-no-loss.sh)'s two trailer lines: atoms (backticked spans, issue refs, link targets, bare URLs) present at base and nowhere now, and how many of those carried a judged entry. Both `null` when the run did not report them, which is never the same as `0` — and that null is the whole point. A run that passed `--claims` and cleared it used to write **the same row** as one that never ran the check, so nothing in the ledger could tell a **verified** [class-C tightening](keep-cut-rubric.md#proving-a-class-c-tightening) from an unverified one. `no_loss_warrants` cannot answer it either: it aggregates every warrant kind, so a `tighten` is indistinguishable from a `retarget` in the count ([#253](https://github.com/gregoryfoster/skills/issues/253)). Both require a `no_loss` verdict, and a non-zero `claims_dropped` requires `failed` — an unwarranted dropped atom exits the check 3, so `ok` beside one is a verdict that run never reached. Recorded, surfaced beside `no_loss` as `/c`, `/c2w`, `/c3d`, and deliberately **not** gated, for the reason [the validation gate](validation-gate.md#the-safety-gates) gives for `no_loss_warrants`. **`0` / `0` is the ordinary shape of a passing run** — most tightenings drop no atom and so warrant none, and `.skills/context-claims-ok` stays absent. Read the pair as *the check ran*; only a non-zero `claims_warranted` says an atom was judged away, and a non-zero `claims_dropped` cannot appear beside `ok` at all |
...
| `counts`, `counts_acked` | [`check-counts.sh`](../scripts/check-counts.sh)'s counts after Phase 6.5's report was judged: **unjudged** rot-prone counts and over-long index lines, and hits warranted in `.skills/context-counts-ok`. Both `null` when the check was not run, which is never the same as `0`. A bare number in a policy file rots silently and no other field on this row can see it: `no_loss` compares claims and not their arithmetic, `seams` sweeps references and a count is not one, and `tokens` reports the total. One cohort policy file carried a `180`-item claim that three parties re-measured as 180, 189 and 182 in one week — none of them wrong, because the sentence never said what was counted — beside five sibling counts correct only by luck, one of which was not ([#258](https://github.com/gregoryfoster/skills/issues/258)). Watch the **delta**, as with `seams_acked`: an acknowledged set that grows every run is a file learning to state numbers it cannot keep. |
