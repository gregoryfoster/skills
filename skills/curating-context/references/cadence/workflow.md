  ...
  measure:
    ...
    steps:
      ...
      - name: Sweep the seams and the counts
        run: |
          bash "$CHECK_SEAMS_SH" --base-ledger ".skills/context-metrics.jsonl" >/tmp/seams.txt 2>&1 || true
      ...
      - name: Commit the row
        run: |
          ...
          git config merge.context-counts.driver "bash \"$MERGE_TOKEN_COUNTS_SH\" %O %A %B %P"
      ...
      - name: Check the recorded deltas
        ...
        run: |
          ...
          if [ -s /tmp/deltas.jsonl ]; then
            echo "::warning::$(wc -l </tmp/deltas.jsonl | tr -d ' ') recorded delta field(s) in .skills/context-metrics.jsonl disagree with the row now before them — usually a curation branch that merged or rebased this one in. On a branch, run \`record-telemetry.sh --repair\` and commit it on its own."
