...
- **`uv sync` prunes groups it was not asked for.** An `ExecStartPre=uv sync` deletes an opt-in dependency group whose test modules `pytest.importorskip` at module scope. Those become skips, not errors: a few hundred tests silently stop running against a suite that still reports green.

The hazard runs the other way too: a worktree's own `uv sync` mutates what the live workers import from. With `none`, `worktree-create.sh` creates no `.venv` and says so on stderr; provision the worktree's own instead — sub-second against a warm cache.

**Tracking.** The knob is read from the **primary checkout**, like `.skills/worktree_root`, so leaving it untracked works: an untracked `.skills/` file does not exist in a linked worktree at all, and reading it from the current checkout would drop the opt-out exactly where it is needed. Commit it only if it holds for every clone — "the main checkout is a service's working directory" is a property of one machine, not of the repository.
