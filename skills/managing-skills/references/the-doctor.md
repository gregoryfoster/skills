...
- **The auto-refresh hook re-installs it** on every session, outside the once-per-day lock, and again right after it advances a pointer, so the doctor it commits with a bump is the one that bump ships ([#299](https://github.com/gregoryfoster/skills/issues/299)). Not in a fresh linked worktree, though: its `skills-vendor/*` are empty gitlinks until something initializes them, so there is no vendored installer to run — and the hook, a symlink into that same tree, does not start there either. When and from which vendor it installs: [auto-refresh-hook.md](auto-refresh-hook.md#installing-the-doctor-twice-and-from-the-first-vendor-that-can).
