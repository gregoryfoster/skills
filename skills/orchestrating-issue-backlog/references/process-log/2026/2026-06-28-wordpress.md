...
**Project:** cannabis.observer-wordpress (InfoSet 5h.x follow-ups). Tracking issue `#416`; plan: a design doc in the consumer repo. Issues #397, #398, #399, #401, #402, #407.
...
**Shape:** 2 waves, 3 parallel agents each, all file-disjoint. Wave A (correctness) = #402, #407, #399. Wave B (reusable/UI) = #397, #398, #401. Only contested file in the whole backlog: the consumer's UI doc (#397+#398, different sections) → intra-batch merge ordering (B1 before B2).
...
- **#407** claimed "check whether the segment transformer / the legislation and rulemaking element transformers share the mis-read." Grep showed the segment and information-set transformers already read the label and suffix fields and clamp to 200 — only the timeline transformer is buggy. Narrowed the agent to one file and added an explicit "out of scope: don't touch the sibling transformers" line so the worker doesn't go spelunking.
- **#399** claimed a "5h.2-5h.5 sweep." Grep of the foreign-key verification helper + `COLLATE utf8mb4_unicode_ci` showed the rulemaking, information-set and timeline-reference migrations already carry the fix; only the legislation and legislative-session migration commands have the check-without-fix. And the timeline and segment migration commands have **no** Check-4 at all (count 0) — a genuine investigation residual, flagged in the plan rather than assumed.
...
- `gh issue create --body-file` (`/tmp/<topic>-tracking-body.md`) — default reach, no heredoc/apostrophe cost.
