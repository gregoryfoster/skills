...
- **#399** claimed a "5h.2-5h.5 sweep." Grep of the foreign-key verification helper + `COLLATE utf8mb4_unicode_ci` showed three other migration commands already carry the fix; only the legislation and legislative-session migration commands have the check-without-fix. And the timeline and segment migration commands have **no** Check-4 at all (count 0) — a genuine investigation residual, flagged in the plan rather than assumed.
