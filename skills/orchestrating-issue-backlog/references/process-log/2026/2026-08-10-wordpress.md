...
Tracking issue `#687`; plan: a design doc in the consumer repo. (Source: upstream issue
...
**[Promotion candidate → Step 7.]** #681 and #666 both regenerate the plugin's committed `openapi.json` via
`dev.sh openapi --write`, and a sync test asserts committed == generated
...
**[Promotion candidate → Step 1–2.]** #656 stated its roles-field blocker "is finding 5 of #655 and
...
theme's label delegation — a different finding entirely — and both `get_field()` reads of the field
were still live, with its key in neither of the guard's retired and deliberate key lists. Unchecked, B1's
...
state of the generated contract*: `openapi.json` already published the create route with
the fields #664 asked for, so one of #664's three claims was closed-in-fact (shipped by #677, `c2d89d22`),
...
happened one step earlier. The conflict analysis identified the testing doc as contested between
three issues and specified an intra-batch merge ordering to manage it. **That file had already been
split into a directory of docs by two merged PRs that were on `origin/main` but not on the local `main` I
...
*references* to the migration journal table across the integration tier, found 15 classes, and reported the
...
their own state-reset helpers; one never set its mid-test commit flag at all. The issue's own 2-class measurement was
...
- **#445 understated by ~2×** (15 classes referencing the migration journal table, 7 with no guarded
...
- **Conflict resolution did not need to go back to a worker.** The one merge conflict
  (a shared test trait) was docblock-only: two workers appending to the same enumerated
...
- **Three-dot/two-dot slip.** Ran a collision check as `git diff --name-only batch/b...656-branch`
  *after* merging that branch into `batch/b`, which yields an empty diff and a false all-clear.
  ...
  false-positive "OVERLAP" from a sloppy grep (one file's name matched as a prefix of another's).
...
- **A CR finding can itself be overstated.** Round 2 item 9 named three test classes; verifying before
  fixing showed only one actually reached the prefetch seam's priming call. Fixed one, said so.
