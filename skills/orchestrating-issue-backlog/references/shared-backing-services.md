...
service the worktrees do not clone — a shared test database above all — or its
third finds a commit hook that runs the suite. Q and
...
**After the provisioning ceiling, ask what hardware runs the verification.** Sub-question 2's
examples — a shared test database, a shared Redis — all name services shared by *state*, and they
steer past a service shared by *capacity*: one VM, or one host, running every worktree's database
and suite. A prior session's **recorded** ceiling is where this bites, because Rule 5 invites a
follow-up session to re-confirm an inherited ceiling cheaply and that shortcut only ever re-reads
the provisioning pool (process-log 2026-09-16 wordpress: "9 (dev.sh port pool)" carried for five
weeks while a 2-CPU / 4-GiB VM capped concurrent verification at **3**; it never bit only because
