...
> 3. **Does the commit hook run the test suite, and at what parallelism?** Multiply that by the agent count and weigh it against the host's cores before accepting a ceiling: [references/shared-backing-services.md](references/shared-backing-services.md).
