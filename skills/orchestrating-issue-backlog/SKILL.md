...
**Backlog provenance predicts where issues *came from*, not whether they're *disjoint*.** Recognizing a backlog's origin front-runs the Step 7 batch shape, but provenance is a prior on geometry, not a guarantee — always confirm via the contested-file grep (Step 5), never substitute the prior for it:
