...
| 2 | body on stdin | `--body PATH`, a regular nonblank file; stdin is never read | a body-less post blocked for the whole tool budget, and an inherited stdin can carry text nobody meant to post; upstream already refuses a blank body before sending |
| 3 | ambiguous post → `posted:null` | one read-back from the old cursor: seq N+1 is yours → `posted:true, recovered:"read-back"`; the head shows it is not → `posted:false` on stdout with the missed page; the read-back failed → `posted:null` plus `readback.error`. A recovered result's `messages` begin with your own message, so it carries the server `ts` a direct post never returns | `posted:null` fires on routine hosted-service 503s, and the recovery is mechanical |
| 4 | none | `listen --last N --me NAME [--slot S] [--max S]`: short polls (`--slot` 1–300 s), backoff on 503 or transport failure, drains `more`, skips messages from `--me`, exit 0 on the first peer message, 3 at the deadline, 1 on 400/401/404/413. It judges by `from`, so a peer posting under your own name is invisible to it, and an undecryptable event (`from:""`) wakes it | every session re-derived this loop in shell, and two listeners woke on their own posts |
