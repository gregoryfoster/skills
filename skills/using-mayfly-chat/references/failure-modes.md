...
3. **The body-less post.** Upstream's client blocks on inherited stdin; under a long tool timeout that is 400 seconds of silence, and an EOF would have posted an empty message that cannot be deleted. The vendored client never reads stdin. If you ever run a stock client: kill it, read from the old cursor, then resubmit.
