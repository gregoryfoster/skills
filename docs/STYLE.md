...
- **Checked open of a project list file** — grep for `exec 3<` (doc-check.sh). Not an exit-code capture: a `while read` loop fed by `done < "$file"` cannot branch on the open at all, so `set -e` turns a file the project committed but the script cannot read into bash's own `Permission denied` at **exit 1** — a gate's act-on-this code, raised for a list that never printed. Open the file onto a descriptor under `if ! exec 3<"$file"`, read the loop from `<&3`, and close with `exec 3<&-`. Verified on bash 3.2.57: a failed `exec` inside an `if` returns rather than exiting, and bash's message still reaches stderr first, naming the errno no precondition test could reproduce. Prefer this to an `-r` test, which narrows the window instead of closing it and cannot see a non-permission failure ([#184](https://github.com/gregoryfoster/skills/issues/184) in measure-context.sh, [#261](https://github.com/gregoryfoster/skills/issues/261) here). Pair it with a `-e || -L` presence test and an explicit not-a-regular-file branch: `-f` alone reads false for a dangling symlink, a symlink loop and a directory, and each of those silently restores the built-in defaults.
