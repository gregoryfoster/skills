...
`version:` records the vendor version last synced from, so the two look redundant and are not: `version:` only moves when the vendor bumps it, and a vendor that edits a `SKILL.md` without bumping leaves the stamps matching over changed text. Every versioned skill in this repo but one had changed since its last bump when [#286](https://github.com/gregoryfoster/skills/issues/286) measured it, so `synced-from:` is the comparand that actually tracks content — the doctor diffs that commit against the submodule's `HEAD`, scoped to the override's own real files, and reads the history to say which side moved: a commit *ahead* of `HEAD` is the pointer lagging, not the override ([#290](https://github.com/gregoryfoster/skills/issues/290)). Bump both on every re-sync. Omitting it warns nothing (for a versioned vendor the stamps still compare); it only gives up the detection.
...
- **Flat at the top level; nested only where a doc links what it nests.** `TestReferences` enforces reachability rather than shape: a top-level reference must be linked *directly* from `SKILL.md`, and anything deeper need only be reachable through a chain of reference links ([#152](https://github.com/gregoryfoster/skills/issues/152)). Every other check over `references/` is recursive for the same reason. Two layouts use that, and they are the only two the rule can express — both put a link on every nested file:
  - an **index** whose entries are many small files, as `orchestrating-issue-backlog`'s process log does;
  - a doc that **annexes one oversized section**, as `curating-context` does for its cadence workflow and its token-measurement chapter — the parent keeps the design and links the annex, which is how a section leaves a full document without costing `SKILL.md` a link it has no tokens for.

  Do not nest for tidiness. The rule is not "subdirectories are allowed"; it is "something reachable must link the file", and a subdirectory nobody links still fails.
