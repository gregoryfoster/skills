...
| `pre-ship-uv-args` | `uv run` arguments, whitespace-separated, any number per line, `#`-comments | `shipping-work-python-fastapi`, `shipping-work-python-click` pre-ship | **extends** every uv call the gate makes, inserted after `uv run` — so the gate runs the suite the project's own hook runs ([#304](https://github.com/gregoryfoster/skills/issues/304)) | a bare `uv run`; present but unreadable is exit 2, never a bare `uv run` |
