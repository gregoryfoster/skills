...
- Two vendoring shapes are in use and they look identical from a shell: a **whole-directory symlink** at `skills/<name>`, and a real directory of **per-file symlinks**. In the second, a file committed as a regular file where the vendor ships one of the same name silently stops tracking upstream forever — `.skills/doctor.sh` reports it as a fork ([#256](https://github.com/gregoryfoster/skills/issues/256)), exempting `pre-ship.sh` by name and anything listed in `.skills/forked-ok`. The remedy is the wrap-don't-fork rule in [STYLE.md](STYLE.md), never a copy.
